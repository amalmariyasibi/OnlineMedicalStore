...
function Products() {
  ...
  const { addToCart, itemCount, loading: cartLoading, error: cartError } = useCart();
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {cartError && (
          <div className="mb-6 bg-red-50 border-l-4 border-red-400 p-4">
            <p className="text-sm text-red-700">Cart Error: {cartError}</p>
          </div>
        )}
                    ...
                    {product.stockQuantity > 0 && currentUser && (
                      <div className="mt-2 grid grid-cols-1 gap-2">
                        <button
                          ...
                          className="w-full bg-blue-600 border border-transparent rounded-md py-2 px-4 flex items-center justify-center text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
        ...
        {itemCount > 0 && (
          <button
            ...
            className="fixed bottom-6 right-6 inline-flex items-center gap-2 px-4 py-3 rounded-full shadow-lg bg-blue-600 hover:bg-blue-700 text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
            aria-label="Open cart"
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
              <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25h10.836c.51 0 .955-.343 1.087-.835l1.5-5.625A1.125 1.125 0 0019.875 6.75H6.72m0 0L5.106 4.835A1.125 1.125 0 004.02 4.125H2.25M6.72 6.75l1.8 6.75M9 20.25a.75.75 0 100-1.5.75.75 0 000 1.5zm9 0a.75.75 0 100-1.5.75.75 0 000 1.5z" />
            </svg>
            <span className="text-sm font-semibold">Cart</span>
            <span className="ml-1 inline-flex items-center justify-center w-6 h-6 text-xs font-bold bg-white text-blue-700 rounded-full">
              {itemCount}
            </span>
          </button>
        )}
